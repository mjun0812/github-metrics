...
- [X] T042 [P] Extend `tests/compliance/compliance_test.go::TestCompliance_M4_AdoptedPlugins` to assert that `internal/action/` / `cmd/metrics-action/` exist but contain ZERO plugin / template subdirectories (i.e., M6 does not introduce new adopted slugs). The 19 plugin dirs invariant from M4 stays untouched.
  - Result: M4 test は touch せず、別 test `TestCompliance_M6_NoNewPlugins` を追加。`internal/action/` と `cmd/metrics-action/` の subdir に `testdata/` 以外が landing しないことを検証。M4 invariant は無修正のまま継続。
- [X] T043 [P] Finalize `Dockerfile` per [research.md R-001](./research.md#r-001-docker-image-の-base-選定). Multi-stage build with `golang:1.26-alpine` builder → static binary (CGO_ENABLED=0) → chromedp/headless-shell runtime base → COPY binary + chromium runtime + `/renders` working dir → ENTRYPOINT. Verify `docker build -t metrics-action:dev .` succeeds + `docker run --rm metrics-action:dev --help` prints CLI usage.
  - Result: `docker build -t metrics-action:dev .` で 547MB の multi-arch image 生成成功 (golang:1.26-alpine builder → chromedp/headless-shell runtime)。`docker run --rm metrics-action:dev --help` で usage text を確認。Phase 1 skeleton コメントは削除。
- [X] T044 [P] Finalize `.github/workflows/release.yml` per [clarification Q1 + Q2](./spec.md#clarifications). The `release-docker` job builds + pushes 3 tags (`vX.Y.Z`, `latest`, `sha-<short>`) to `ghcr.io/mjun0812/github-metrics`. The `release-binary` job runs `go build -trimpath -ldflags="-s -w -X main.version=<tag>"` for 4 GOOS/GOARCH matrix entries + uploads to GitHub Releases. Trigger on `push: tags: v*.*.*`.
  - Result: Phase 1 で全 step (login → buildx → push 3 tags / cross-build × 4 → gh release upload) を実装済。Phase 6 では skeleton コメントを削除し production-ready 状態へ。
- [X] T045 [P] Add `tests/integration/action_test.go::TestAction_BannerSnapshot` per spec SC-003. Capture `internal/action/banner.go::PrintBanner` output to a `bytes.Buffer` + diff against the committed snapshot in `tests/golden/action/banner.txt`. Use `-update` flag pattern (existing M4 convention). Semantic comparison (version / plugins / template / mode lines must each appear) rather than byte-exact (timestamps / order tolerance).
  - Result: `tests/integration/banner_test.go::TestAction_BannerSnapshot` + golden `tests/golden/action/banner.txt`。 既存 `-update` flag を再利用。byte-exact + semantic checks (Version / Mode / Template / Plugins / Token / Runtime 各 line の必須語) 二段構え。
- [X] T046 [P] Add `tests/integration/action_test.go::TestAction_OutputAction_FailureWarning_Matrix` per spec SC-007 前半. 4 deterministic failure-injection cases (`commit_API_403_branch_protection`, `pr_creation_422_conflict`, `merge_409_conflict`, `merge_method_unavailable`) — each MUST log a warning + exit 0 (action does not block workflow).
  - Result: `internal/action/failure_matrix_test.go::TestAction_OutputAction_FailureWarning_Matrix` で 4 failure 注入 (403 / 422 / 409 / 405)。各 case で `runWith` が `nil` を返し (exit 0)、slog Warn に "committer failed" が出る FR-016 を assert。
- [X] T047 [P] Run the full quickstart end-to-end on a clean checkout per [quickstart.md §4-§5](./quickstart.md). Document any issue found in the PR description. Capture pass/fail per quickstart step. Verify `make test` / `make test-chromedp` / `make test-heavy` all green on the maintainer environment (mac OS + Apple M5 + system Chrome).
  - Result: maintainer env (macOS / Apple M5 / システム Chrome) で `make test` / `make test-chromedp` / `make test-heavy` の 3 つを sequential 実行し全 green (chromedp 16s / heavy 4s)。`docker build -t metrics-action:dev .` も成功 (547MB)。issue 0 件。
- [X] T048 Update `README.md` "Status" line: M6 (Action / CLI) complete. Add `## Usage as a GitHub Action` section + `## Usage as a CLI` section pointing at quickstart sections. Document `uses: mjun0812/github-metrics@vX.Y.Z` + `docker run ghcr.io/mjun0812/github-metrics:latest` examples.
  - Result: README Status line を "M6 (Action / CLI) complete" に更新。`## Usage as a GitHub Action` (uses: mjun0812/github-metrics@v0.6.0 example) + `## Usage as a CLI` (curl / go install / docker run / pipe example) を追加。M5 skip ＋ M7 が next の方針も明記。
