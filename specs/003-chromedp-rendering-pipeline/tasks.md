...
- [X] T035 [US2] Vendor 3 upstream fixture cases under `tests/fixtures/upstream/svg_hash/`: for each, the `.svg` is a copy of the input the upstream `svg.hash` consumed, and the `.hash` is the upstream-produced 32-char hex string. Reuse `internal/tools/sync-fixtures` (M2) extended with a `--mode svg-hash` flag to drive the generation from `./org_repo/source/app/metrics/utils.mjs`. Make T033 pass. **Scoped out of M3 — tracked as follow-up issue [#113](https://github.com/mjun0812/github-metrics/issues/113)**: vendoring requires running upstream Node code under `./org_repo` (puppeteer + chromium ~300 MB install) and carries a high probability of DOM-serializer drift between puppeteer's `outerHTML` and goquery's pure-Go `OuterHtml`. The Go-side `TestUpstreamHashCompatible` scaffold already `t.Skip`s gracefully when the fixture directory is absent, so M3 ships as 54/54 with the upstream-compat assertion deferred to #113 (blocks M6 T-114). Mitigation paths if the fixtures land but mismatch: (a) per-case fixture massaging, or (b) research R-013's chromedp-driven hash fallback (trade-off: M6 T-114 then requires chromedp at runtime).
