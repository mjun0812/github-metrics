...
- [ ] T035 [P] Add `tests/compliance/compliance_test.go::TestCompliance_M7_TemplateInvariant` per [research.md R-008](./research.md#r-008-compliance-test-extension). Enumerate `internal/templates/` subdirectories (exclude `partials/`) and assert the set equals `{classic, repository}`. Pair with `TestCompliance_M4_AdoptedPlugins` for the dual-invariant defense.
