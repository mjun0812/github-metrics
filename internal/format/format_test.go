...
func TestContributionLevel(t *testing.T) {
	...
	tests := []struct {
		...
		// Color path: canonical GitHub palette wins regardless of count.
		{"color L0", 99, "#ebedf0", 0},
		{"color L1", 0, "#9be9a8", 1},
		{"color L2", 0, "#40c463", 2},
		{"color L3", 0, "#30a14e", 3},
		{"color L4", 0, "#216e39", 4},
		// Case insensitive + whitespace tolerant on color match.
		{"color uppercase", 0, "#9BE9A8", 1},
		{"color trimmed", 0, "  #40C463  ", 2},
		// Count fallback when color is empty / unknown.
		{"count 0 -> L0", 0, "", 0},
		{"count 1 -> L1", 1, "", 1},
		{"count 3 -> L1", 3, "", 1},
		{"count 4 -> L2", 4, "", 2},
		...
		{"count 10 -> L4", 10, "", 4},
		{"count 100 -> L4", 100, "", 4},
		// Unknown color falls through to count.
		{"unknown color falls back", 5, "#abcdef", 2},
		// Negative counts clamp to L0.
		{"negative count", -3, "", 0},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := format.ContributionLevel(tt.count, tt.color); got != tt.want {
				t.Fatalf("ContributionLevel(%d, %q) = %d, want %d", tt.count, tt.color, got, tt.want)
			}
		})
	}
}
