...
// the upstream xml-formatter `collapseContent=true` knob). The pipeline
// feeds it pure SVG, so self-closing is always well-formed.
//
...
func FormatXML(in string) (string, error) {
	...
	for {
		...
		case xml.EndElement:
			...
			if pendingOpen {
				// Empty element: collapse to a self-closing `<tag/>`
				// (upstream collapseContent=true). Well-formed for the
				// SVG the pipeline emits.
				buf.WriteString("/>\n")
				pendingOpen = false
				continue
			}
