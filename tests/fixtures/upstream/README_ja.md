...
lowlighter/metrics の参照 JSON を保管する。

## 21-plugin baseline (`octocat.json`)

M4 Polish (T092) 時点では未生成である。生成手順:

1. `cd org_repo && npm install` (upstream の dev dependencies。約 1 GB)
2. `org_repo/tests/cases/octocat.yml` を作成する。21 採用 slug (P1 5 / P2 12 /
   P3 4 = topics / starlists + languages.recent / languages.indepth)
   すべてを有効化した case を定義する。`languages.recent` /
