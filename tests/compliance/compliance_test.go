...
var adoptedM4Plugins = []string{
	...
	"topics", "starlists",
	// #602: header extracted from the legacy base plugin. base/ was
	// deleted in #605; header is a regular adopted plugin from the
...
func TestCompliance_M4_AdoptedPlugins(t *testing.T) {
	...
	if len(missing) == 0 && len(extra) == 0 {
		t.Logf("M4 採用 20 plugin compliance OK (dirs: %d adopted + core)", len(have))
	}
}

// TestCompliance_DocsPluginPagesMatchAdoptedSet asserts that the set
// of `docs/plugins/*.md` pages matches the 20 user-facing adopted
// plugins. The infrastructure plugin `core` is a foundational
// component: it ships its own doc page describing its inputs
// (see `internal/tools/gen-plugin-docs/main.go::foundationalSlugs`),
// but it is NOT part of the adopted-20 gating set — the README
