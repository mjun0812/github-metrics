# Reference examples (`lowlighter/metrics`, real data)

This directory contains reference cards generated with
...
- [`docs/org_examples`](../org_examples) (= `original_examples`) is a straight import of
  `lowlighter/metrics`'s `examples` branch, whose **data subject is lowlighter himself**. It can be
  used for structural comparison, but since the data differs, it does not allow an
  apples-to-apples comparison of "how do `lowlighter/metrics` and the Go implementation differ given
  the same input."
- This directory is the output of feeding **the same data (mjun0812 /
  flash-attention-prebuild-wheels)** into `lowlighter/metrics`. Placing it side by side with
...
| Item              | Value                                                                                                                                               |
| ----------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| Tool              | `lowlighter/metrics@v3.34` (`lowlighter/metrics` itself, GitHub Action)                                                                             |
...
> Warning: these are **raw**, non-normalized `lowlighter/metrics` output. Dynamic parts such as the
...
**error on the `lowlighter/metrics` v3.34 side** caused `Unexpected error` to be rendered on the card
...
`lowlighter/metrics` tool, GitHub API spec changes, or permission/data issues.
...
| `metrics.plugin.activity.svg`             | An `lowlighter/metrics` code bug. `TypeError: Cannot read properties of undefined (reading 'filter')` (`source/plugins/activity`) causes `Unexpected error`.                                                                                         |
| `metrics.repository.plugin.activity.svg`  | Same as above (activity in the repository template).                                                                                                                                                                                                 |
| `metrics.plugin.habits.facts.svg`         | An `lowlighter/metrics` code bug. `TypeError: Cannot destructure property 'author' of 'undefined'` (`source/plugins/habits/index.mjs:51`) causes `Unexpected error`.                                                                                 |
...
> projects could not be rendered due to `lowlighter/metrics` v3.34 errors and were removed (see
...
> activity / traffic could not be rendered due to `lowlighter/metrics` v3.34 errors and insufficient
...
> Warning: this does not work on self-hosted runners. The `lowlighter/metrics` action mounts
...
To regenerate just one card locally, you can run the `lowlighter/metrics` image directly. Set
`GITHUB_TOKEN` (a PAT with roughly `repo` / `read:user` scope) as an environment
variable, and pass `INPUT_*` to the `lowlighter/metrics` image. Note that values must be url-encoded
with `jq @uri` when passed (`lowlighter/metrics`'s `metadata.mjs` decodes them with
