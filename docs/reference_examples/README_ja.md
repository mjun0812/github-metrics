...
- [`docs/org_examples`](../org_examples) (= `original_examples`) は upstream の
  ...
  構造比較には使えるが、データが違うため「同じ入力で upstream と Go 実装がどう違うか」
...
> ⚠️ これらは正規化していない **生の** upstream 出力である。フッターの生成タイムスタンプや
...
した。いずれも本プロジェクトの Go 実装の不具合ではなく、upstream ツールのコードバグ /
...
> projects は upstream v3.34 のエラーで描画できず削除した (上記「正しく描画できなかった
> カード」参照)。
...
> activity / traffic は upstream v3.34 のエラー / データ不足で描画できず削除した
...
> ⚠️ self-hosted runner では動かない。upstream action が `--volume $GITHUB_EVENT_PATH`
...
1枚だけ手元で再生成する場合は upstream イメージを直接実行できる。`GITHUB_TOKEN`
(`repo` / `read:user` 程度のスコープを持つ PAT) を環境変数に設定し、upstream イメージへ
`INPUT_*` を渡す。値は `jq @uri` で url-encode して渡す点に注意する必要がある (upstream の
