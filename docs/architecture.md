...
`github-metrics` は GitHub ユーザー / 組織 / リポジトリの統計を集約し、プロフィールカードを SVG / PNG / JPEG / JSON で出力するツールである。upstream [`lowlighter/metrics`](https://github.com/lowlighter/metrics) の subset を Go に移植したもので、採用範囲は [`scope.md`](scope.md) に定義する。
...
入力の命名体系とフラグ一覧は [`configuration.md`](configuration.md) を参照。パースした入力は `internal/action.Run` に渡され、`engine.Compute` を呼ぶ。
...
レンダリングの詳細 (native SVG の高さ確定、resvg、装飾ステージ) は [`rendering.md`](rendering.md) を参照。
...
採用プラグインの一覧と分類は [`scope.md`](scope.md) を参照。

## 4. テンプレート

登録テンプレートは **`classic`** (user / organization) と **`repository`** (単一リポジトリ) の 2 つのみ。`terminal` / `markdown` は不採用 ([`scope.md`](scope.md#31-web-インスタンス-旧-m5))。
...
`Run` は partial 群を縦に積み上げて 1 枚の SVG 文字列を返す。partial のシグネチャは高さ自己申告付きの `(markup string, height int, err error)` で、詳細は [`rendering.md`](rendering.md#2-partial-と高さの確定) にある。
